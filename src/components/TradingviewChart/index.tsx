...
const TradingViewChart = ({
  ...
  useEffect(() => {
    if (data !== dataPrev && chartCreated && type === CHART_TYPES.BAR) {
      ...
      let tooltip: any = document.getElementById('tooltip-id' + type)
      let node: any = document.getElementById('test-id' + type)
  ...
  const formattedData = data?.map((entry: any) => {
    return {
      time: moment(entry.ts*1000).format('YYYY-MM-DD'),
  ...
  useEffect(() => {
    if (!chartCreated && formattedData && ref.current && data.length > 0 && baseChange !== '') {
  ...
  return (
    <Wrapper>
      <div ref={ref as any} id={'test-id' + type} />
      <IconWrapper>
        <Play
          onClick={() => {
