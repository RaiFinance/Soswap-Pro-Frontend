import { Pair,Currency } from 'zksdk'
...
const CurrencySelect = styled.button<{ selected: boolean }>`
  ...
  cursor: pointer;
  user-select: none;
  border: 1px solid ${({ selected, theme }) => (selected ? theme.text4 : theme.text1)};
  padding: 0 0.5rem;
            ...
            <Aligner>
              ...
              {pair ? (
                <StyledTokenName className="pair-name-container">
                  {changeSymbol(chainId, pair?.token0.symbol)}:{changeSymbol(chainId, pair?.token1.symbol)}
                </StyledTokenName>
              ) : (
                <StyledTokenName className="token-symbol-container" active={Boolean(currency && currency.symbol)}>
                  {(currency && currency.symbol && currency.symbol.length > 20
                    ...
                    :changeSymbol(chainId, currency?.symbol)) || t('selectToken')}
                </StyledTokenName>
              )}
